...
public class SalleServiceImpl
    ...
    @Override
    @Transactional
    public List<ResponseSoutenanceDTO> affecterSalles(LocalDate dateDebut) {

        List<Jury>  tousJurys = juryRepository.findAllWithRelations();
        ...
        soutenanceRepository.deleteAllSoutenances();
    ...
    public List<ResponseSalleDTO> importFromExcel(InputStream is) throws Exception {
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (nom.isBlank() || salleRepository.existsByNomSalle(nom)) continue;
            nouvelles.add(Salle.builder()
                    .nomSalle(nom).capacite(cap).disponible(true).build());
        }
        wb.close();
    ...
    // ══════════════════════════════════════════════════════════════
    @Override
    public LocalDate importDateDebut(InputStream is) throws Exception {
        Workbook wb    = new XSSFWorkbook(is);
